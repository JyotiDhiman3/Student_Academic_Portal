...
public class facultycli {
    ...
    private static void viewGradesOfAllStudents(Connection connection) throws SQLException {
        Scanner scanner = new Scanner(System.in);
    ...
    private static boolean isValidFaculty(String email_id, String password) {
...
private static void registerForOfferedCourse(Connection connection, Scanner scanner, String emailId) throws SQLException {
    ...
    double cgpaCriteria = scanner.nextDouble();
    scanner.nextLine();
    ...
    String sql = "INSERT INTO instructor (instructor_id, course_code_offered, cgpa_criteria, semester) VALUES (?, ?, ?, ?)";
    try (PreparedStatement stmt = connection.prepareStatement(sql)) {
        stmt.setString(1, emailId);
        stmt.setString(2, courseCode);
        stmt.setDouble(3, cgpaCriteria);
...
private static void deregisterFromOfferedCourse(Connection connection, Scanner scanner, String emailId) throws SQLException {
    ...
    String sql = "DELETE FROM instructor WHERE instructor_id = ? AND course_code_offered = ? AND semester = ?";
    try (PreparedStatement stmt = connection.prepareStatement(sql)) {
        stmt.setString(1, emailId);
        stmt.setString(2, courseCode);
...
private static boolean isCourseCodeValid(Connection connection, String courseCode) throws SQLException {
    String sql = "SELECT * FROM coursecatalog WHERE course_code = ?";
    try (PreparedStatement stmt = connection.prepareStatement(sql)) {
...
private static boolean isOfferingCourseInSemester(Connection connection, String emailId, String courseCode, int semester) throws SQLException {
    String sql = "SELECT * FROM instructor WHERE instructor_id = ? AND course_code_offered = ? AND semester = ?";
...
private static void updateCourseGradesFromFile(Connection connection, String filePath) throws SQLException {
    try {
